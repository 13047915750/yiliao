...
<script>
import { resetPasswordByOld } from '@/api/account'
...
export default {
  ...
  methods: {
    ...
    async resetPasswordByOld() {
      await resetPasswordByOld({ ...this.form, token: this.$store.getters.userInfo.token })
      this.$message({
        type: 'success',
        message: '修改成功,请重新登录'
      })
